...
calc_j_popavg <- function(K, L, max.J, psbl.seq, intcpt, slp, 
                          corstr, corr.resid.err, sigma.resid.err, 
  ...
  if (is.na(max.J)) {
    ...
    while (actual.power < power) {
      ...
      actual.power <- calc_power(K, L, J, psbl.seq, intcpt, slp, 
                                 corstr, corr.resid.err, sigma.resid.err, 
                                 var.intcpt, var.slp, cov.intcpt.slp,
                                 type.1.err, min.diff, file.psbl.seq)
    }
  ...
  res <- c(J, K, L, actual.power)
  names(res) <- c("J", "K", "L", "power")
  
  return(res)
}

# Find L (number of measurements per period) for pts graph
calc_l_popavg <- function(J, K, max.L, max.KL, psbl.seq, intcpt, slp,
                          corstr, corr.resid.err, sigma.resid.err, 
                          var.intcpt, var.slp, cov.intcpt.slp,
                          type.1.err, power, min.diff,
                          file.psbl.seq = NULL) {
  ...
  if (is.na(max.L)) {
    ...
    if (slp == "common") {
      ...
      while ((actual.power < power) & ((L + 1) <= floor(max.KL / K))) {
      ...
      } # else {} if (actual.power < power) {
      
    } # (slp == "random") start from the largest L
...
calc_power <- function(K, L, J, psbl.seq, intcpt, slp, 
  ...
  if (psbl.seq == "Alternating Sequences") {
    ...
    if ((intcpt == "fixed") & (slp == "common")) {
      actual.power <- calc_power_alter_fixintcpt_fixslp(K, L, J, Sigma.eps, type.1.err, min.diff)
      
    } else if ((intcpt == "fixed") & (slp == "random")) {
      D <- var.slp
      actual.power <- calc_power_alter_fixintcpt_randslp(K, L, J, Sigma.eps, D, type.1.err, min.diff)
      
    } else if ((intcpt == "random") & (slp == "common")) {
      D <- var.intcpt
      actual.power <- calc_power_alter_randintcpt_fixslp(K, L, J, Sigma.eps, D, type.1.err, min.diff)
      
    } else if ((intcpt == "random") & (slp == "random")) {
      D <- matrix(c(var.intcpt, cov.intcpt.slp, cov.intcpt.slp, var.slp), ncol = 2)
      actual.power <- calc_power_alter_randintcpt_randslp(K, L, J, Sigma.eps, D, type.1.err, min.diff)
    }
    
  } else if (psbl.seq == "Pairwise Randomization") {
    
    if ((intcpt == "fixed") & (slp == "common")) {
      actual.power <- calc_power_pair_fixintcpt_fixslp(K, L, J, Sigma.eps, type.1.err, min.diff)
      
    } else if ((intcpt == "fixed") & (slp == "random")) {
      D <- var.slp
      actual.power <- calc_power_pair_fixintcpt_randslp(K, L, J, Sigma.eps, D, type.1.err, min.diff)
      
    } else if ((intcpt == "random") & (slp == "common")) {
      D <- var.intcpt
      actual.power <- calc_power_pair_randintcpt_fixslp(K, L, J, Sigma.eps, D, type.1.err, min.diff)
      
    } else if ((intcpt == "random") & (slp == "random")) {
      D <- matrix(c(var.intcpt, cov.intcpt.slp, cov.intcpt.slp, var.slp), ncol = 2)
      actual.power <- calc_power_pair_randintcpt_randslp(K, L, J, Sigma.eps, D, type.1.err, min.diff)
    }
    
  } else if (psbl.seq == "User-Specified Sequences") {
    
    if ((intcpt == "fixed") & (slp == "common")) {
      actual.power <- calc_power_other_fixintcpt_fixslp(K, L, J, Sigma.eps, type.1.err, min.diff, file.psbl.seq)
      
    } else if ((intcpt == "fixed") & (slp == "random")) {
      D <- var.slp
      actual.power <- calc_power_other_fixintcpt_randslp(K, L, J, Sigma.eps, D, type.1.err, min.diff, file.psbl.seq)
      
    } else if ((intcpt == "random") & (slp == "common")) {
      D <- var.intcpt
      actual.power <- calc_power_other_randintcpt_fixslp(K, L, J, Sigma.eps, D, type.1.err, min.diff, file.psbl.seq)
      
    } else if ((intcpt == "random") & (slp == "random")) {
      D <- matrix(c(var.intcpt, cov.intcpt.slp, cov.intcpt.slp, var.slp), ncol = 2)
      actual.power <- calc_power_other_randintcpt_randslp(K, L, J, Sigma.eps, D, type.1.err, min.diff, file.psbl.seq)
    }
...
calc_power_alter_fixintcpt_fixslp <- function(K, L, J, Sigma.eps, type.1.err, min.diff) {
...
calc_power_alter_randintcpt_fixslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff) {
...
calc_power_alter_fixintcpt_randslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff) {
...
calc_power_alter_randintcpt_randslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff) {
...
# 2) fixed intercept; fixed slope
calc_power_pair_fixintcpt_fixslp <- function(K, L, J, Sigma.eps, type.1.err, min.diff) {
...
# 1) pairwise randomization
# 2) random intercept; fixed slope
calc_power_pair_randintcpt_fixslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff) {
...
# 1) pairwise randomization
# 2) fixed intercept; random slope
calc_power_pair_fixintcpt_randslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff) {
...
# 1) pairwise randomization
# 2) random intercept; random slope
calc_power_pair_randintcpt_randslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff) {
...
# Calculate the power for 
# 1) user-specified sequences 
# 2) fixed intercept; fixed slope
calc_power_other_fixintcpt_fixslp <- function(K, L, J, Sigma.eps, type.1.err, min.diff, file.psbl.seq) {
...
# 1) user-specified sequences
# 2) random intercept; fixed slope
calc_power_other_randintcpt_fixslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff, file.psbl.seq) {
...
# 1) user-specified sequences
# 2) fixed intercept; random slope
calc_power_other_fixintcpt_randslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff, file.psbl.seq) {
...
# 1) user-specified sequences
# 2) random intercept; random slope
calc_power_other_randintcpt_randslp <- function(K, L, J, Sigma.eps, D, type.1.err, min.diff, file.psbl.seq) {
